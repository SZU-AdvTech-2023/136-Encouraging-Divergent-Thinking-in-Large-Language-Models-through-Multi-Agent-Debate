...
sys.path.append('/public24_data/wzy2023/OpenAI-Parallel-Toolkit') 
import json
import logging
import traceback
...
class Debate:
    ...
    def init_agents(self):
        ...
        ## 添加了baseprompt捏
        #self.Baseline.set_meta_prompt(self.save_file['base_SQL_query'])
...
if __name__ == "__main__" :
    ...
    #key_manager = KeyManager(config_path="/public24_data/wzy2023/Multi-Agents-Debate/config.json")
    key_manager = KeyManager(config_path="/public24_data/qth/chatgpt_test/config.json")
    #key_manager = KeyManager(config_path="/public24_data/qth/chatgpt_test/config4.json")
    ...
    ## 循环使用key，把所有key直接放sh里面，然后循环使用
            ...
            while attempts < max_retries:
                ...
                # Attempt to generate a completion
                    res = func_timeout(120.0, execute_model)
                    ...
                    key_manager.release_key(openai_api_key)
                    openai_api_key = key_manager.get_new_key()
                    logging.error(f"{LOG_LABEL}key {openai_api_key} ,request timeout")
                    continue
